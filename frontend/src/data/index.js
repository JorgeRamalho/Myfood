export const brand = {
  ...
  stores: {
    // Placeholders até publicação nas lojas — instalação real via PWA em /baixar
...
/** Restaurante sem menu embutido (formato da API). */
export function toApiRestaurant(restaurant) {
  if (!restaurant) return null;
  const { menu: _menu, ...rest } = restaurant;
  return rest;
}
...
export function filterLocalRestaurants({ category, q } = {}) {
...
export function getLocalFeaturedRestaurants() {
  return restaurants.filter((item) => item.featured).map(toApiRestaurant);
}
